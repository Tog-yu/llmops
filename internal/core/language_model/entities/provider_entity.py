...
class Provider(BaseModel):
    ...
    def validate_provider(cls, provider: dict[str, Any]) -> dict[str, Any]:
        ...
        for model_type in provider_entity.supported_model_types:
            ...
            # ModelType 为 str Enum 时，f-string 会得到「ModelType.xxx」而非模块名「chat」，需取 .value
            type_slug = model_type.value if isinstance(model_type, ModelType) else model_type
            symbol_name = type_slug[0].upper() + type_slug[1:]
            provider["model_class_map"][model_type] = dynamic_import(
                f"internal.core.language_model.providers.{provider_entity.name}.{type_slug}",
                symbol_name
            )
